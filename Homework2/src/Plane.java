...
public class Plane {
	...
	Plane() {
		...
		Human[][] seatsHuman = new Human[27][6];
	}
	...
	public void add(Human h) {
		while(capacity<162){
			int gSize2 = gSize;
			int rvalue = new Random().nextInt(gSize) + 1;//gSize -> number of people groups allowed to get in, every value lower than num than is allowed by default)
			if(capacity+rvalue > 162) continue;//so we can't add more people than max number of seats
			capacity = capacity + rvalue;
			iCase(rvalue); //Equals group as the argument for iCase()
			if ( gSize != gSize2 ) capacity = capacity - rvalue; //Checks-in if group of people happened to be kicked out of the plane. Point is that if we don't substract the number of the people in that group, we'd get inaccurate result for |sum|.
			System.out.println(" ");
			System.out.println( rvalue + " passenger(s) enters the airplane." );
		//	print_seats();
			System.out.println(capacity); //prints number of seats that are already taken
		}		
	}
